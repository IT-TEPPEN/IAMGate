...
class TestServiceActionPostgreSQLRepository:
    ...
    def test_save_and_get_service_action(
        self,
        repository: ServiceActionPostgreSQLRepository,
        sample_service_action: ServiceAction,
    ):
    ...
    def test_get_service_action_not_found(
        self, repository: ServiceActionPostgreSQLRepository
    ):
        """存在しないService Actionの取得をテスト"""
        result = repository.get_service_action("nonexistent:action")
        assert result is None

    def test_save_service_actions_bulk(
        self, repository: ServiceActionPostgreSQLRepository
    ):
    ...
    def test_update_service_action(
        self,
        repository: ServiceActionPostgreSQLRepository,
        sample_service_action: ServiceAction,
    ):
    ...
    def test_update_service_action_not_found(
        self, repository: ServiceActionPostgreSQLRepository
    ):
        ...
        with pytest.raises(
            ValueError, match="Service Action with id nonexistent:Action not found"
        ):
            repository.update_service_action(action)

    def test_delete_service_actions_before(
        self, repository: ServiceActionPostgreSQLRepository
    ):
        ...
        # 2024年以前のアクションを削除
        deleted_count = repository.delete_service_actions_before(
    ...
    def test_get_service_actions_with_criteria(
        self, repository: ServiceActionPostgreSQLRepository
    ):
        ...
        criteria = VServiceActionSearchCriteria.new(
            service_name="s3", limit=10, offset=0
        )
        ...
        # アクセスレベルで検索
        criteria = VServiceActionSearchCriteria.new(
            access_level=EAccessLevel.読み込み, limit=10, offset=0
        )
