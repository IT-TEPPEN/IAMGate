...
class ServiceActionPostgreSQLRepository(IServiceActionRepository):
    ...
    def update_service_action(self, action: ServiceAction) -> ServiceAction:
        ...
        with Session(self.engine) as session:
            ...
            db_action.resource_types = action.resource_types if action.resource_types else None
            db_action.condition_keys = action.condition_keys if action.condition_keys else None
    ...
    def delete_service_actions_before(self, updated_before: datetime) -> int:
        ...
        with Session(self.engine) as session:
            statement = select(MServiceAction).where(MServiceAction.updated_at < updated_before)
            result = session.exec(statement)
    ...
    def _build_search_query(self, criteria: VServiceActionSearchCriteria, count_only: bool = False):
        """検索条件からクエリを構築"""
        if count_only:
        ...
        if criteria.service_name:
            ...
            conditions.append(func.lower(MServiceAction.service_prefix).like(f"%{criteria.service_name.lower()}%"))
        ...
        if criteria.action_name:
            if criteria.action_name_match == EActionNameMatch.exact:
                ...
                from sqlalchemy import func
                conditions.append(func.lower(MServiceAction.action_name).like(f"%{criteria.action_name.lower()}%"))
            elif criteria.action_name_match == EActionNameMatch.regex:
                # 正規表現の長さをチェック（安全のため）
                if not criteria.validate_regex_pattern():
                    raise ValueError("Regular expression pattern is too long (max 500 characters)")
                # PostgreSQLの正規表現演算子を使用
                conditions.append(MServiceAction.action_name.op("~")(criteria.action_name))
        
        if criteria.description:
            from sqlalchemy import func
            conditions.append(func.lower(MServiceAction.description).like(f"%{criteria.description.lower()}%"))
        
        if criteria.access_level:
            conditions.append(MServiceAction.access_level == criteria.access_level.value)
    ...
    def _apply_pagination_and_sort(self, statement, criteria: VServiceActionSearchCriteria):
        """ページネーションとソートを適用"""
        # ソート
